...
public class PublicCandidateSubmissionRequestValidator : AbstractValidator<PublicCandidateSubmissionRequest>
{
    public PublicCandidateSubmissionRequestValidator()
    {
        Include(new CreateCandidateRequestValidatorAdapter());
    }
}

internal sealed class CreateCandidateRequestValidatorAdapter : AbstractValidator<PublicCandidateSubmissionRequest>
{
    public CreateCandidateRequestValidatorAdapter()
    {
        RuleFor(x => x.Name)
            .NotEmpty().WithMessage("Nome é obrigatório")
            .MaximumLength(255).WithMessage("Nome não pode exceder 255 caracteres");
